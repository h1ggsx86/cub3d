...
/*   Updated: 2025/03/27 14:48:09 by tnedel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mouse_move(t_game *g, t_mouse m)
{
	...
	{
		way = -1;
		dist = g->win_width - m.x;
		dist /= 10;
		double oldDirX = p->dirX;
		p->dirX = p->dirX * cosf(ROT_SPEED / 2) - p->dirY * sinf(ROT_SPEED / 2);
		p->dirY = oldDirX * sinf(ROT_SPEED / 2) + p->dirY * cosf(ROT_SPEED / 2);
		double oldPlaneX = p->viewX;
		p->viewX = p->viewX * cosf(ROT_SPEED / 2) - p->viewY * sinf(ROT_SPEED / 2);
		p->viewY = oldPlaneX * sinf(ROT_SPEED / 2) + p->viewY * cosf(ROT_SPEED / 2);
		redraw_img(g);
	}
	else
	{
		way = 1;
		dist = m.x - g->win_width;
		dist /= 10;
		double oldDirX = p->dirX;
		p->dirX = p->dirX * cosf(-ROT_SPEED / 2) - p->dirY * sinf(-ROT_SPEED / 2);
		p->dirY = oldDirX * sinf(-ROT_SPEED / 2) + p->dirY * cosf(-ROT_SPEED / 2);
